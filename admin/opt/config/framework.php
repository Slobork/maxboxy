<?php
	...
	CSF::createOptions( $prefix,
		array(
			 ...
			 * If set as main 'menu' and post types (inject_any, float_any) are added to it, setting 'show_sub_menu' => false - won't work, 
			 * i.e. it only lists all Tabs of the Options panel as submenus
			 * ...so, that's why I have set 'admin_menu' for it, @see Max__Boxy::admin_menu(). It acts as a parent of this submenu.
			 */
			'menu_type'  		=> 'submenu',
			'menu_parent'		=> 'maxboxy-settings', // created at @see Max__Boxy::admin_menu().
			/**
			 * 'menu_title' => esc_html__( 'Settings', 'maxboxy' ), // this won't work, 
			 * instead it uses the title of the registered admin menu item from the Max__Boxy::admin_menu().
			 */
	...
	/**
	 * You can also add a class to any field to hide it in the future, if there's a need.
	 * ...If the Pro version is deactivated, in order to keep its settings,
	 * just add the "empty_class" to hide the field when its just the base plugin active
	 */
	...
	// if the splitter plugin isn't active
	} else {

		$splitter_module = array(

		// The following should be set once the Splitter is developped and the $empty_field removed:
		/* 
			array(
				'type'      => 'subheading',
