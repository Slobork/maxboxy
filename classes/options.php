	...
	if ( ! class_exists( 'Max__Boxy__Options' ) ) {
		...
		class Max__Boxy__Options {
			...
			public static function loading( $get_id ) {
				...
				$asign_metabox	=	get_post_type( $get_id ) === 'float_any' || get_post_type( $get_id ) === 'inject_any'
								?	true	:	false;

				if ($asign_metabox === false) return;

				/**
				 * *_loading Meta box's data is unserialized, so get it directly without the prefix,
				 * i.e. not using _mb_floatany like _mb_floatany_loading | _mb_injectany_loading.
				 * ...Just use its fields directly, e.g. 'location' (location is under _loading mtabox as well)
				 */
			...
			public static function basics( $get_id ) {
				...
				$asign_metabox	=	get_post_type( $get_id ) === 'wp_block' && class_exists( 'Max__Boxy__Reusable_blocks' ) && Max__Boxy__Reusable_blocks::enabled() === true
				...
				// Popup style
				$showing_style                  =   get_post_type( $get_id ) === 'float_any' ?	' style-bumpin'	:	' style-onload'; // default -> bumpin for FloatAny and onload for InjectAny
				...
				// rotator and igniter are checked multiple times below, since make the condtion check here
				...
				// additional classes - the field is in the group (i.e. in tab), so we need to sanitize it here
				 ...
				 */
				$_escaped_panel_content_style   =   ! empty( $panel_padding ) || ! empty ( $panel_border ) || ! empty ( $panel_border_radius ) || ! empty( $set_panel_bg ) || ! empty( $panel_color )
			...
			public static function conversions( $get_id ) {
				...
				$goalset                        =   ! empty( $goal )	?	' goalset goal-' .$goal   :    '';

				$goal_after_banish              =   ! empty( $goal ) && ! empty( $conversions[ 'goal_after_banish' ] )    ?   ' goal-after-banish'  :   '';
