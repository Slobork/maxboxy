<?php
	...
	/**
	 * Add columns for specified post types listing.
	 */
	if ( ! class_exists( 'Max__Boxy__Admin_Columns' ) ) {

		// float_any post type
		add_filter( 'manage_float_any_posts_columns', 			array ( 'Max__Boxy__Admin_Columns',	'add_admin_columns' ) );
		add_action( 'manage_float_any_posts_custom_column', 	array ( 'Max__Boxy__Admin_Columns', 	'admin_custom_columns_data' ) );
		
		// inject_any post type
		add_filter( 'manage_inject_any_posts_columns', 			array ( 'Max__Boxy__Admin_Columns',	'add_admin_columns' ) );
		add_action( 'manage_inject_any_posts_custom_column', 	array ( 'Max__Boxy__Admin_Columns', 	'admin_custom_columns_data' ) );
		...
		if ( class_exists( 'Max__Boxy__Reusable_blocks' ) && Max__Boxy__Reusable_blocks::enabled() === true ) {
			...
			add_filter( 'manage_wp_block_posts_columns', 		array ( 'Max__Boxy__Admin_Columns',	'add_admin_columns' ) );
			add_action( 'manage_wp_block_posts_custom_column', 	array ( 'Max__Boxy__Admin_Columns', 	'admin_custom_columns_data' ) );
