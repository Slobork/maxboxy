<?php
	...
	if ( ! function_exists( 'maxboxy_upgrade_call' ) ) {
		...
		function maxboxy_upgrade_call() {
			...
			if ( class_exists( 'Max__Boxy__Pro' ) && $get_license === false ) {
				...
				/**
				 * @see _safe_license_notactive_notice() from Max__Boxy class - all esacped there
				 */
			...
			// else would be called if the Pro version isn't active at all
				...
				echo '<div style="padding: 0 10px; border-left: 4px solid #339fd4;">' .__( 'Upgrade to ', 'maxboxy') .'<a href="https://maxpressy.com/floatany/" target="_blank">' .__( 'Pro version', 'maxboxy' ) .'</a> to gain access to premium features.</div>';
			}
	 ...
	 *
	 * Displays the stats. Used in Max__Boxy__Track::reset_panel_stats() as well.
	 *
	...
	if ( ! function_exists( 'maxboxy_stats_call' ) ) {
		...
		function maxboxy_stats_call( $id ) {
			...
			// $id passed from @see maxboxy_splitters_stats(), otherwise it's the current post id
			...
			if ( $get_license !== false && function_exists( 'maxboxy_stats_call_pro' ) ) {
				...
				// Only for the 'post' page, i.e. omit for the listing (i.e. post type page), but not on the splitter's post type
